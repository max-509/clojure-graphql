...
(defquery get-persons "create (a:Person:Manager {name: 'Emil' from: 'Sweden' klout: 99 list: [99 0 1] listt: []})-[b:FRIEND $A]->(c:Person)
                      match (a:Person {name: 'Emil'})-[]->(b:Person) where a.name = 'Emil' AND a.klout < 100 AND NOT (a.klout > 50 OR a.from = 'Sweden')")
